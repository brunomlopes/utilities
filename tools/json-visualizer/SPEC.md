...
- Filter 12.1. Suffixing one or more `^` characters to a bare property name pulls the property up by that many containing-object levels: `^` pulls it one level up, `^^` pulls it two levels up, and so on. Array containers do not count as levels. If the requested number of levels extends beyond the output root, the destination is clamped to the output root. Pulling removes ancestor branches that are empty after filtering.
