...
- Append `^` to pull a property one ancestor level up, `^^` to pull it two levels up, and so on. For example, against `{ "a": { "b": { "enabled": true } } }`, `a[b[enabled^]]` produces `{ "a": { "enabled": true } }`, while `a[b[enabled^^]]` produces `{ "enabled": true }`. Excess carets clamp the destination at the output root.
